...
const testimonialsList = computed(() => props.testimonials.data);
...
function deleteTestimonial() {
    if (testimonialToDelete.value) {
        deleteForm.delete(route('admin.testimonials.destroy', testimonialToDelete.value.id), {
            onSuccess: () => {
                showDeleteModal.value = false;
                testimonialToDelete.value = null;
            }
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-white shadow-sm">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b pb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Liste des témoignages</h2>
            ...
            <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <div v-for="testimonial in testimonialsList" :key="testimonial.id" class="bg-white border rounded-xl shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                    <!-- Image de l'auteur -->
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        <h3 class="text-lg font-semibold text-gray-800 mb-1">{{ testimonial.author_name }}</h3>
                        <p v-if="testimonial.author_title" class="text-sm text-gray-500 mb-3">{{ testimonial.author_title }}</p>
                        <p class="text-gray-600 text-sm mb-4 flex-1">
                            <span>
                                {{ stripAndTruncateHtml(testimonial.content, 120) }}
                            </span>
                        </p>
                ...
                <div v-if="testimonialsList.length === 0" class="col-span-full bg-gray-50 rounded-lg p-10 text-center">
                    <svg xmlns="http://www.w3.org/2000/svg" class="h-16 w-16 text-gray-400 mx-auto mb-4" fill="none" viewBox="0 0 24 24" stroke="currentColor">
                        <path stroke-linecap="round" stroke-linejoin="round" stroke-width="2" d="M8 10h.01M12 10h.01M16 10h.01M9 16H5a2 2 0 01-2-2V6a2 2 0 012-2h14a2 2 0 012 2v8a2 2 0 01-2 2h-5l-5 5v-5z" />
                    </svg>
                    <h3 class="text-lg font-medium text-gray-900 mb-2">Aucun témoignage trouvé</h3>
                    <p class="text-gray-500 mb-4">Commencez par ajouter un nouveau témoignage de client.</p>
                    <Link
                        :href="route('admin.testimonials.create')"
            ...
            <div v-if="viewMode === 'list'" class="overflow-x-auto rounded-lg border border-gray-200">
                <table class="min-w-full bg-white divide-y divide-gray-200">
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="testimonial in testimonialsList" :key="testimonial.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4 whitespace-nowrap">
                                <div v-if="testimonial.author_title" class="text-sm text-gray-600">
                            ...
                            <td class="px-6 py-4">
                                <div class="text-sm text-gray-600 max-w-xs">
                                    {{ stripAndTruncateHtml(testimonial.content) }}
                                </div>
                            </td>
                        ...
                        <tr v-if="testimonialsList.length === 0">
                            <td colspan="6" class="px-6 py-10 text-center">
                                ...
                                <p class="text-gray-500">Aucun témoignage trouvé. Commencez par en ajouter un !</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <div class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6">
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Affichage de <span class="font-medium">{{ props.testimonials.from }}</span> à
                            <span class="font-medium">{{ props.testimonials.to }}</span> sur
                            <span class="font-medium">{{ props.testimonials.total }}</span> témoignages
                        </p>
                    </div>
                    <div>
                        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                            <Link
                                v-for="(link, i) in props.testimonials.links"
                                :key="i"
                                :href="link.url || '#'"
                                v-html="link.label"
                                :class="[
                                    ...
                                    i === 0 ? 'rounded-l-md' : '',
                                    i === props.testimonials.links.length - 1 ? 'rounded-r-md' : '',
                                    'border'
                                ]"
                            />
                        </nav>
                ...
                <div class="flex flex-1 justify-between items-center sm:hidden">
                    <Link
                        :href="props.testimonials.links[0].url || '#'"
                        :class="[
                            'relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md',
                            props.testimonials.links[0].url === null
                    ...
                    <Link
                        :href="props.testimonials.links[props.testimonials.links.length - 1].url || '#'"
                        :class="[
                            'relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md',
                            props.testimonials.links[props.testimonials.links.length - 1].url === null
                                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                : 'bg-white text-gray-700 hover:bg-gray-50',
                            'border border-gray-300'
                        ]"
    ...
    <div v-if="showDeleteModal" class="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50">
        <div class="bg-white p-6 rounded-lg shadow-lg max-w-md w-full">
            ...
            <p class="mb-6 text-gray-600 text-center">
                Êtes-vous sûr de vouloir supprimer le témoignage de "<span class="font-medium">{{ testimonialToDelete?.author_name }}</span>" ?
