...
const isPaginated = computed(() => {
    return typeof (props.services as Pagination).current_page !== 'undefined';
});

// Accéder aux services en fonction de la structure
const servicesList = computed(() => {
    if (isPaginated.value) {
        return (props.services as Pagination).data;
    }
    return props.services as Service[];
});
...
function deleteService() {
    if (serviceToDelete.value) {
        window.location.href = route('admin.services.destroy', serviceToDelete.value.id);
    }
    showDeleteModal.value = false;
}
    ...
    <AppLayout :breadcrumbs="breadcrumbs">
        <div class="flex h-full flex-1 flex-col gap-6 rounded-xl p-6 bg-white shadow-sm">
            ...
            <div class="flex flex-col sm:flex-row sm:justify-between sm:items-center gap-4 border-b pb-6">
                <div>
                    <h2 class="text-2xl font-bold text-gray-800">Liste des services</h2>
            ...
            <div v-if="viewMode === 'grid'" class="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 xl:grid-cols-4 gap-6">
                <div v-for="service in servicesList" :key="service.id" class="bg-white border rounded-xl shadow-sm hover:shadow-md transition overflow-hidden flex flex-col">
                    ...
                    <div class="p-4 flex-1 flex flex-col">
                        ...
                        <p class="text-gray-600 text-sm mb-4 flex-1">
                            <span v-if="service.description">
                                {{ stripAndTruncateHtml(service.description, 120) }}
                            </span>
                ...
                <div v-if="servicesList.length === 0" class="col-span-full bg-gray-50 rounded-lg p-10 text-center">
                    ...
                    <p class="text-gray-500 mb-4">Commencez par ajouter un nouveau service à votre catalogue.</p>
                    <Link
                        :href="route('admin.services.create')"
                ...
                <table class="min-w-full bg-white divide-y divide-gray-200">
                    <thead class="bg-gray-50">
                        <tr>
                            ...
                            <th scope="col" class="px-6 py-3 text-left text-xs font-medium text-gray-500 uppercase tracking-wider">Prix min.</th>
                    ...
                    <tbody class="bg-white divide-y divide-gray-200">
                        <tr v-for="service in servicesList" :key="service.id" class="hover:bg-gray-50 transition">
                            ...
                            <td class="px-6 py-4">
                                <div v-if="service.description" class="text-sm text-gray-600 max-w-xs">
                                    {{ stripAndTruncateHtml(service.description) }}
                                </div>
                        ...
                        <tr v-if="servicesList.length === 0">
                            <td colspan="6" class="px-6 py-10 text-center">
                                ...
                                <p class="text-gray-500">Aucun service trouvé. Commencez par en ajouter un !</p>
                            </td>
                        </tr>
                    </tbody>
                </table>
            ...
            <!-- Pagination -->
            <div v-if="isPaginated" class="flex items-center justify-between border-t border-gray-200 px-4 py-3 sm:px-6">
                <div class="hidden sm:flex-1 sm:flex sm:items-center sm:justify-between">
                    <div>
                        <p class="text-sm text-gray-700">
                            Affichage de <span class="font-medium">{{ (props.services as Pagination).from }}</span> à <span class="font-medium">{{ (props.services as Pagination).to }}</span> sur <span class="font-medium">{{ (props.services as Pagination).total }}</span> services
                        </p>
                    </div>
                    <div>
                        <nav class="isolate inline-flex -space-x-px rounded-md shadow-sm" aria-label="Pagination">
                            <Link
                                v-for="(link, i) in (props.services as Pagination).links"
                                :key="i"
                                :href="link.url || '#'"
                                v-html="link.label"
                                :class="[
                                    ...
                                    i === 0 ? 'rounded-l-md' : '',
                                    i === (props.services as Pagination).links.length - 1 ? 'rounded-r-md' : '',
                                    'border'
                                ]"
                            />
                        </nav>
                ...
                <div class="flex flex-1 justify-between items-center sm:hidden">
                    <Link
                        :href="(props.services as Pagination).links[0].url || '#'"
                        :class="[
                            'relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md',
                            (props.services as Pagination).links[0].url === null
                    ...
                    <Link
                        :href="(props.services as Pagination).links[(props.services as Pagination).links.length - 1].url || '#'"
                        :class="[
                            'relative inline-flex items-center px-4 py-2 text-sm font-medium rounded-md',
                            (props.services as Pagination).links[(props.services as Pagination).links.length - 1].url === null
                                ? 'bg-gray-100 text-gray-400 cursor-not-allowed'
                                : 'bg-white text-gray-700 hover:bg-gray-50',
                            'border border-gray-300'
                        ]"
